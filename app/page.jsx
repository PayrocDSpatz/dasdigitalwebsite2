...
/** Single-file landing page for DAS Digital. Requires Tailwind + lucide-react. */
  ...
  <div className="relative overflow-hidden">
    <div className="flex w-max animate-[marquee_18s_linear_infinite] gap-6 py-2">
      {items.concat(items).map((t, idx) => (
        <span key={`${t}-${idx}`} className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/80">
          {t}
        </span>
      ))}
    </div>
...
const Card = ({ icon, title, children }) => (
  <div className="rounded-3xl border-2 border-purple-500 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_20px_40px_-25px_rgba(168,85,247,0.45)] backdrop-blur">
...
export default function Page() {
  const pain = useMemo(
    () => ["Too many manual steps", "Leads not followed up consistently", "Dashboards that don’t match reality", "Tools that don’t talk to each other", "Busy teams, slow execution"],
    []
  );

  const purpleBtn = "bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white shadow-lg shadow-purple-500/30 hover:from-purple-500 hover:to-fuchsia-500";
    ...
    <div className="min-h-screen bg-[#070910] text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#070910]/70 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <a href={brand.primaryCta.href} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold ${purpleBtn}`}>
      ...
      <main>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
              ...
              <p className="mt-5 text-pretty text-base leading-relaxed text-white/70 sm:text-lg">
                {brand.tagline} We audit what you do today, remove friction, then deploy a done-for-you stack (Smartsheet, Make/Zapier, CRM, email, reporting) with documentation so you own the playbook.
              </p>
              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                <a href={brand.primaryCta.href} className={`inline-flex items-center justify-center gap-2 rounded-2xl px-6 py-3 text-sm font-semibold ${purpleBtn}`}>
                  {brand.primaryCta.label} <ArrowRight className="h-4 w-4" />
                </a>
              ...
              <div className="mt-10 space-y-4">
                <MarqueeRow
                  items={["Stop manual busywork", "Instant routing & approvals", "AI email follow-ups", "Dashboards that match reality", "Human-in-the-loop safety"]}
                />
              </div>
            </div>
            ...
            <div className="relative">
              <div className="rounded-[2rem] border-2 border-purple-500 bg-gradient-to-b from-white/10 to-white/5 p-6 shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_30px_70px_-30px_rgba(168,85,247,0.6)]">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <div className="rounded-3xl border border-white/10 bg-black/20 p-4">
                    ...
                    <div className="mt-3 flex h-10 items-end gap-1">
                      {["40%","65%","55%","80%","60%"].map((h,i)=>(
                        <div key={i} className="w-3 rounded bg-purple-400/70" style={{height:h}} />
                      ))}
                    </div>
                  </div>
                  <div className="rounded-3xl border border-white/10 bg-black/20 p-4">
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6" id="what">
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-2">
            <div className="rounded-3xl border-2 border-purple-500 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_20px_40px_-25px_rgba(168,85,247,0.45)]">
              <div className="text-sm font-semibold text-white">Typical “agency / consultant” model</div>
              <ul className="mt-4 space-y-3 text-sm text-white/70">
                {["Sells deliverables, not results", "Outsources execution", "Hands you reports and disappears", "No documentation or ownership"].map((t) => (
            ...
            <div className="rounded-3xl border-2 border-purple-500 bg-gradient-to-b from-white/10 to-white/5 p-6 shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_20px_40px_-25px_rgba(168,85,247,0.45)]">
              <div className="text-sm font-semibold text-white">DAS Digital model</div>
              <ul className="mt-4 space-y-3 text-sm text-white/80">
                {["We embed with your team and execute", "Automate end-to-end (capture → approvals → updates → notifications)", "AI-assisted follow-ups with human checks", "Documentation + handoff so you own the playbook"].map((t) => (
                  <li key={t} className="flex items-start gap-2">
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6" id="proof">
          <SectionTitle kicker="Proof" title="Before → After outcomes you can feel" subtitle="Swap chaos for clarity. These are common results once the engine is installed (your numbers will vary)." />
          <div className="mt-10 grid gap-6 lg:grid-cols-3">
            {[
              ...
              <div key={x.before} className="rounded-3xl border-2 border-purple-500 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_20px_40px_-25px_rgba(168,85,247,0.45)]">
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6" id="process">
          <SectionTitle kicker="How it works" title="A simple, done-for-you process" subtitle="We move fast, document everything, and leave you with a system you can run in-house." />
          <div className="mt-10 grid gap-6 lg:grid-cols-3">
            {[
              { step: "1", title: "Audit + roadmap", desc: "We map your workflows, data sources, and bottlenecks. You get a prioritized plan with quick wins + a 30/60/90 build path." },
              { step: "2", title: "Build + integrate", desc: "We implement automations, dashboards, and AI-assisted communications with guardrails, permissions, and testing." },
              { step: "3", title: "Handoff + optimize", desc: "Documentation, SOPs, training, and iterative improvements. You own the playbook — no vendor lock-in." },
            ].map((s) => (
              <div key={s.step} className="rounded-3xl border-2 border-purple-500 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_20px_40px_-25px_rgba(168,85,247,0.45)]">
                <div className={`inline-flex h-9 w-9 items-center justify-center rounded-2xl text-white font-semibold shadow shadow-purple-500/40 bg-gradient-to-r from-purple-600 to-fuchsia-600`}>
                  {s.step}
                </div>
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6" id="lead">
          <div className="rounded-[2.25rem] border-2 border-purple-500 bg-gradient-to-b from-white/10 to-white/5 p-8 backdrop-blur shadow-[0_0_0_1px_rgba(168,85,247,0.25),0_30px_60px_-30px_rgba(168,85,247,0.55)] sm:p-10">
            <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
              <div>
                ...
                <p className="mt-3 text-white/70">Tell us what you’re trying to automate (or what’s breaking). We’ll respond with a clear blueprint: tools, steps, timeline, and rough ROI.</p>
                <ul className="mt-5 space-y-2 text-sm text-white/75">
                  {["No fluff. Clear next actions.", "Includes quick wins + longer-term build.", "Guardrails + security considerations."].map((t) => (
                    <li key={t} className="flex items-start gap-2">
                      <span className="mt-0.5 grid h-5 w-5 place-items-center rounded-full bg-white text-black">
                        <Check className="h-3 w-3" />
                      </span>
              ...
              >
                <div className="grid gap-4">
                  ...
                  <button className={`inline-flex items-center justify-center gap-2 rounded-2xl px-6 py-3 text-sm font-semibold ${purpleBtn}`} type="submit">
                    Send request <ArrowRight className="h-4 w-4" />
                  </button>
                  <p className="text-xs text-white/60">Prefer a link button? Replace this with your Typeform/Calendly URL.</p>
                </div>
              </form>
            </div>
          </div>
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6" id="faq">
          ...
          <div className="mt-10 grid gap-4">
            <FAQItem q="Do you work with Smartsheet + Make.com?" a="Yes — those are two of our core tools. We also work with Zapier, Google Workspace, Microsoft 365, HubSpot/Salesforce, QuickBooks, and custom APIs when needed." />
            <FAQItem q="Will we own everything you build?" a="Yes. You own the automations, sheets, docs, prompts, and SOPs. We design for handoff so you’re not dependent on us long-term." />
            <FAQItem q="How fast can we see impact?" a="Most teams see quick wins in days (routing, notifications, data validation). Larger multi-system builds typically take 2–8 weeks depending on scope." />
            <FAQItem q="Can you add AI safely without sending sensitive data everywhere?" a="Yes. We can implement approvals, redaction, role-based controls, and ‘human in the loop’ steps. For sensitive environments, we’ll suggest the safest architecture available." />
          </div>
        </section>
          ...
          <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
            <div className="grid gap-6 md:grid-cols-2 md:items-center">
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-3 text-sm text-white/70">
                  ...
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2">
                    ...
                    <a href="tel:19546733041" className="text-purple-400 hover:text-purple-300 underline-offset-2 hover:underline">
                      (954) 673-3041
                    </a>
                  </span>
                </div>
              ...
              <div className="md:text-right">
                <a href={brand.primaryCta.href} className={`inline-flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold ${purpleBtn}`}>
                  {brand.primaryCta.label} <ArrowRight className="h-4 w-4" />
                </a>
